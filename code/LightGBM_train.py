...
def train_model(dtrain, dvalid, categorical_features='auto'):
    model_params = {
            'objective': CFG.objective, # 이진 분류
            'boosting_type': CFG.boosting_type,
            'metric': CFG.metric, # 평가 지표 설정
            'feature_fraction': CFG.feature_fraction, # 피처 샘플링 비율
            'bagging_fraction': CFG.bagging_fraction, # 데이터 샘플링 비율
            'bagging_freq': CFG.bagging_freq,
            'lambda': 0.7401250246820457,
            'learning_rate': 0.06163827164757535,
...
def make_lgb_oof_prediction(train, y, test, features, categorical_features='auto', folds=10):
...
def main():
    ...
    y_oof, test_preds, fi = make_lgb_oof_prediction(train, y, test, features, folds=CFG.folds)
